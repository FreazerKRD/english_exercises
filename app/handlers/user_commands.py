...
async def start_command(message: types.Message, **kwargs):
    ...
    await message.answer(f"Приветствую, {usr_name}!\n \n \
                         ...
                         Ограничения: формат книги TXT, размер файла не более 300 кбайт. \n \
                         Для загрузки книги просто перетащи её в этот чат.")
...
async def books_process(callback: types.CallbackQuery, 
    ...
    await callback.message.edit_reply_markup(books_kb(books_list, start_index=_start_idx, books_per_page=books_per_page))
